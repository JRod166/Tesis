...
public:
	...
	*/
	...
	void calibrate_camera_iterative(int n_iterations, int n_frames, int grid_rows, int grid_cols, string name) {
		...
		for (int i = 0; /*abs(this_rms-prev_rms) > threshold*/ i<2 ; i++) {
			prev_rms = this_rms;
			iteration++;
...
double CameraCalibration::calibrate_camera() {
	...
	//cout << "set_points size: " << set_points.size() << endl;
	...
	///rvecs es el vector de rotacion de 3 elementos
	///tvecs es el vector de traslacion de 3 elementos 
	/*cout << endl;
